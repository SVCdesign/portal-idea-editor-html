...
rem primeira vez neste PC: instala a pecinha do robo (usa o Chrome ja instalado)
if not exist "node_modules\playwright-core\package.json" (
  echo   Preparando pela primeira vez neste PC. Aguarde alguns segundos...
...
echo   estiver usando o editor. Para fechar, feche esta janela.
echo.
...
rem liga o motorzinho Node (serve o editor + atende o "Gerar PNG")
node server.mjs

echo.
echo   ---------------------------------------------
echo   O servidor parou. Se apareceu algum erro acima,
echo   tire um print e mostre ao assistente.
echo   ---------------------------------------------
pause
